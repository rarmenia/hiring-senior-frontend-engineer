              ...
              {(pie) => pie.arcs.map((arc) => (
                <g
                  ...
                  <path
                    ...
                    fill={props.colors[props.indexAccessor(arc.data) % props.colors.length]}
                    className={classnames('transition-all', 'ease-in', 'duration-75', 'cursor-pointer')}
                    style={props.indexAccessor(active) === props.indexAccessor(arc.data) ? {filter: `drop-shadow(0px 0px 2px ${props.colors[props.indexAccessor(arc.data) % props.colors.length]})`} : {}}
                    onClick={() => setActive((cur) => props.indexAccessor(cur) === props.indexAccessor(arc.data) ? undefined : arc.data)}
                  />
                </g>
              ))}
            </Pie>
            {(active !== undefined && props.displayText) &&
        ...
        <div className={classnames('mt-4', 'md:ml-2', 'w-full', 'md:w-2/3', 'flex', 'flex-row', 'items-center')}>
          <table className={classnames('w-full')}>
            ...
            {props.data.sort((a, b) => props.valueAccessor(b) - props.valueAccessor(a)).map((dataRow, currIndex) => (
                <tr key={props.indexAccessor()}
                    onClick={() => setActive((cur) => props.indexAccessor(cur) === props.indexAccessor(dataRow) ? undefined : dataRow)}
                  ...
                  <td className={classnames('w-2/3', 'inline-flex', 'items-center', theme.chartLegendKey)}>
                    <div className={classnames('block', 'rounded-full', 'mt-0.5')} style={{
                      backgroundColor: props.colors[props.indexAccessor(dataRow) % props.colors.length],
                      height: '6px',
                      width: '6px'
                    }}/>
